;Modified by Toby Herman to ask for a number rather than text, June 2008
;Otherwise, this is the original program 'textbox' by David Fanning
;
;
;+
; NAME:
...
;
; PURPOSE:
;  This function allows the user to type a floating point number in a
;  pop-up dialog widget and have it returned to the program.
;
; CALLING SEQUENCE:
;
;  frametime = SMF_GUI_Frametime()
;
; INPUTS:
;
;  None.
...
;
;       frametime = SMF_GUI_Frametime(Title='Provide Phone Number...', Label='Number:', Cancel=cancelled)
...
;
;          frametime = SMF_GUI_Frametime(Group_Leader=event.top)
;
...
;
;  VALUE: A variable that is the intial value of the box. By default, zero.
;
;  XSIZE: The size of the text widget in pixel units. By default, 200.
;
; OUTPUTS:
;
;  Frametime: The frametime the user typed in the widget. No error checking is done.
;
; RESTRICTIONS:
;
;  The widget is destroyed if the user clicks on either button or
;  if they hit a carriage return (CR) in the text widget. The
;  number is recorded if the user hits the ACCEPT button or hits
;  a CR in the widget.
;
...
CASE event.ID OF
   ...
   ELSE: BEGIN
         ...
         ; Get the number and store it in the pointer location.
...
FUNCTION SMF_GUI_Frametime, Title=title, Label=label, Cancel=cancel, $
...
IF N_Elements(value) EQ 0 THEN value = ""  ELSE value = String(value)
...
Widget_Control, tlb, Set_UValue=info, /No_Copy
    ...
    ;Keyboard focus on accept button
Widget_Control, acceptID, /Input_Focus
